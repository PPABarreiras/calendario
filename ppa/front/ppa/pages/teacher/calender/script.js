  ...
  times.forEach((time) => {
    ...
    li.addEventListener('click', function onClick() {
      if(!li.children.length) {
        ...
        if(!time.captured) {
          const getVacantClass = document.createElement('button');
          getVacantClass.classList.add('buttonCaptureVacantClass');
          getVacantClass.textContent = 'Pegar';
          getVacantClass.addEventListener('click', capturedVacantClass);
          div2.appendChild(getVacantClass);
        }
...
const buttonAddNewTask = document.querySelector('#addTask');
